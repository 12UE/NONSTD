...
public:
	...
	//重载[]运算符
    RBTreeNode* operator[](const int nData);
...
private:
    RBTreeNode* root;
	RBTreeNode* nil;//nil 是一个空节点，它的左右孩子和父亲都是nil，它的颜色是黑色
    int    node_count;
};
...
inline RBTreeNode* RBTree::Find(const int nData)
{
    RBTreeNode* pCurrent = root;
...
inline RBTreeNode* RBTree::operator[](const int nData){
	return Find(nData);
}

...
private:
	...
	inline void SetSystemTimerReslutionBegin(fclock_t _fResolution) {
	...
	inline void SetSystemTimerReslutionEnd(fclock_t _fResolution) {
		if (m_bNatieApiOK) {
			ULONG m_DesiredReslution = (ULONG)(10000 * _fResolution);
...
public:
	...
	inline fclock_t duration(timepoint _begin, timepoint _end) { return TimeCast(_end - _begin); }
	inline fclock_t operator()() { return duration(m_begin, now()) / 1000.0f; }
	...
	inline timepoint after(timepoint now, fclock_t millisecond) {
	...
	inline void HighResolutionSleep(fclock_t  _millisecond, std::string name = "") {//睡眠毫秒
		...
		if (resttime > 0) {
			resttime = std::clamp<fclock_t>(resttime, 0, 5000);//0-5s内 
			LARGE_INTEGER start, now;
			QueryPerformanceCounter(&start);
			do {
				QueryPerformanceCounter(&now);
			} while ((m_predictor(now.QuadPart) - start.QuadPart) * 1e+6 / fclock_t(perfCnt.QuadPart) < resttime);
		}
	}
};
...
static auto _initial_time = GlobalClock();
